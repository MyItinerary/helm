...
export default function HeaderTheme({ currentPreferredTheme }: { currentPreferredTheme: Theme }) {
  ...
  const changePreferredTheme = useCallback((t: Theme) => {
    ...
    if (t === Theme.Auto) {
      Cookies.set('theme', window.matchMedia('(prefers-color-scheme: dark)').matches ? Theme.Dark : Theme.Light)
      router.refresh()
      return
    }
  ...
  return (
    <Dropdown>
      ...
      <DropdownMenu className="pt-0" align="end">
        <DropdownItem active={preferredTheme === Theme.Light} onClick={() => changePreferredTheme(Theme.Light)}>
          <FontAwesomeIcon className="me-2" icon={faSun} fixedWidth />
          Light
        </DropdownItem>
        <DropdownItem active={preferredTheme === Theme.Dark} onClick={() => changePreferredTheme(Theme.Dark)}>
          <FontAwesomeIcon className="me-2" icon={faMoon} fixedWidth />
          Dark
        </DropdownItem>
        <DropdownItem active={preferredTheme === Theme.Auto} onClick={() => changePreferredTheme(Theme.Auto)}>
